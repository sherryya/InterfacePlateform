...
import com.main.AutoClacCanStream;
...
public class CronAutoClacMileage implements Job {
	...
	private void autoRun() {
		...
		for (ZdcItovTerminal zdc : zdcItovTerminal) {
			...
			if (list != null && list.size()>0) {
			   ...
			   for (ZdcCanstream l : list) {
				   if(l!=null)
				   {
						...
						if(temp == 2)
						{
							...
							oilBegin = l.getResidualOilConsumption(); //开始剩余油耗
			   ...
			   if(mileStart!=null && mileEnd!=null)
			   {
				   mileage = mileEnd - mileStart;
				   zdcMile.setDatastreamid2(String.valueOf(mileage));//行驶里程
			   }
